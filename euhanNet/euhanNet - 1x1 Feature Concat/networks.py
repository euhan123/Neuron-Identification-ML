...
class euhanNet(nn.Module):
    def __init__(self, load_weights = False):
        ...
        self.seen = 0
        self.scale_3x3 = [32,32,64,64,64]
        self.scale = make_layers(self.scale_3x3, in_channels = 3)
        self.scale_1x1 = [32,32,64,64,64]
        self.scale_org = make_layers(self.scale_1x1, in_channels = 3)
        self.concat = [64]
        self.postconcat = make_layers(self.concat, in_channels = 64)
        self.front_feat = [128, 128, 'M', 64, 64, 'M', 64, 32] #setting up the layers (M = max-pooling)
        self.front = make_layers(self.front_feat, in_channels = 128)
        self.output_layer = nn.Conv2d(32,1, kernel_size = 1) #create output layer (convert back to 2 channel)
    ...
    def forward(self, x1, x2):
        x1_ = self.scale(x1)
        x2_ = self.scale_org(x2)
        x2_ = torch.cat((x2_,x2_,x2_), 2)
        x2_ = torch.cat((x2_,x2_,x2_), 3)
        x2_ = self.postconcat(x2_)
        ...
        zeroes = torch.zeros(s)
        zeroes = zeroes.to(device)
        ones = torch.ones(s)
        ones = ones.to(device)
        x_ = torch.where(x > 0.5, ones, zeroes)
        x = F.interpolate(x, size = x1.size()[2:], mode = 'bilinear', align_corners=True)
        x_ = F.interpolate(x_, size = x1.size()[2:], mode = 'bilinear', align_corners=True)
        return x, x_
