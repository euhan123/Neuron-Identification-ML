...
parser.add_argument('--name', required=False, default='euhanNet_1x1 Feature Concat',  help='')
...
def train(train_loader_src, model, optimizer, device, losses, train_hist, ce_loss):
    for data in train_loader_src:
        ...
        output, output_ = model(img3x3, img1x1)
        loss = OPloss(output.float(), output_.float(), gt.float())
...
def validate(test_loader_src, model, device, epoch):
    ...
    for data in test_loader_src:
        ...
        output_, output = model(img3x3, img1x1)
...
def OPloss (output, output_, target):
    #output = torch.argmax(output, 1)
    output = output.squeeze()
    output_ = output_.squeeze()
    #output = output.double()
    loss = 3*(output - target)*output_ + (1-output_)*(target - output)
