import h5py #what is this?
import torch
import shutil
from skimage.transform import resize
import numpy as np

#help me very hard to understand
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'): #when do u use save_net v/s save_checkpoint
...
def genScalePos(imSize, ph, pw): #what is this scaling?
    h = imSize[2]
    w = imSize[3]
...
def cropMultiScale(im, N_scale):
    ...
    the scale is depend on img shape and number of scales
...
def resizeDensityPatch(patch, opt_size): #resize the density map
    '''
    ...
    # Normalize values between 0 and 1. It is in order to perform a resize.
    p_max = patch.max()
    p_min = patch.min()
    # Avoid 0 division
    if patch_sum != 0:
